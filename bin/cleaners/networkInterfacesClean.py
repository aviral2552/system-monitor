...
def initiator(dataDir, machineID, logPath):
	...
	os.chdir(dataDir)
	if os.path.exists('networkInterfaceLogs.xlsx') == False:
		addHeaders()
	else:
		emptyLists()
	...
	if os.path.exists('networkInterfaceLogs.xlsx') == True:
		oldFile = load_workbook('networkInterfaceLogs.xlsx')
		writer = pd.ExcelWriter('networkInterfaceLogs.xlsx', engine='openpyxl')
		writer.book = oldFile
		...
		writer = pd.ExcelWriter('networkInterfaceLogs.xlsx')
